...
def run_client_action(credential: Credential, action: Callable[[BossClient], T]) -> T:
    ...
    try:
        ...
        # __zp_stoken__ is regenerated by browser JS, so re-reading the browser's cookies is the
        # only way to refresh it. Keep the saved credential when that fails: its login cookies
        # are still valid for endpoints that do not check __zp_stoken__ (e.g. boss me).
        from ..auth import browser_extraction_failure_reason, extract_browser_credential
        fresh, diagnostics = extract_browser_credential()
        if fresh:
            with get_client(fresh) as client:
                return action(client)
        # Surface why the refresh failed so the user knows which fix is theirs to make.
        raise SessionExpiredError(browser_extraction_failure_reason(diagnostics)) from None
